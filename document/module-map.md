...
| app | backend-wake-gate | shell global (antes de `AuthProvider`) | no visible | active | `document/deployment.md` | `project-overview` | `app/BackendWakeGate.jsx` | Poll a `/actuator/health` con mensajes de espera mientras el backend (Render free) hace cold start; deja pasar tras 75s aunque no responda. |
