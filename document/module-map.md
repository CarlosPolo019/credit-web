...
| users | users | `/users` | Usuarios (solo `ADMIN`) | active | `pages/users/README.md` | — | `pages/users/UsersPage.jsx` | Crea cuentas `USER` de prueba (comerciales) via `POST /api/v1/users`, protegido con `DashboardLayout` + `AdminRoute`. Sin listar/editar/eliminar — el backend no lo expone. Sin skill dedicado aun. |
| assistant | lesson-dock | shell autenticado | Copiloto (toggle) | active | `pages/assistant/README.md` | `assistant` | `pages/assistant/AssistantDock.jsx` | Columna derecha de ensenanza en `DashboardLayout`. Sin ruta propia, sin LLM. Primera sesion: 3 notas y se oculta (`credit-web-lesson-beats`). |
